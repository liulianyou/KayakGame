...
bool UCheckGameOverFailed_AllPlayerDead::RegisterPawn(APawn* Pawn)
{
	...
	if (Pawn->GetClass()->ImplementsInterface(UDeadSupportInterface::StaticClass()))
	{
		...
		{
			UE_LOG(LogEvaluator, Warning, TEXT("Please make the native pawn class inherited form interface:IDeadSupportInterface, Or CheckGameOver_Faild condition will always return true "));
		}
	}
